#!/usr/bin/env node
/**
 * Photos v5: location-biased town-vibe photos.
 *
 * Over v4:
 *   - First fetches the station's lat/lng via Google Places Text Search
 *     for "{name}駅".
 *   - Each subsequent Places Text Search for landmark queries is biased
 *     to that lat/lng with a 2km radius. Excludes candidates far from the
 *     actual station even if they share the name.
 *   - Same strict rating/type/regex filtering as v4.
 *
 * Usage: node scripts/enrich-photos-v5.mjs [limit]
 */
...
// Names that look like offices/firms despite good ratings — filter out
const NAME_BAD_REGEX =
  /(税理士|法律事務所|法人|事務所|会計|保険|クリニック|医院|歯科|整骨|整体|動物病院|薬局|弁護士|司法書士|社労士|駐車場|不動産|リフォーム|塾|予備校|スクール|教室|葬儀|斎場|支店|営業所)/;
...
async function collectCandidates(stationName, loc) {
...
async function main() {
  ...
  let processed = 0, enriched = 0, thin = 0, noLoc = 0;
  ...
  for (const t of towns) {
    ...
    try {
      ...
      if (!loc) noLoc++;
      const candidates = await collectCandidates(t.name, loc);
      ...
      if (processed % 25 === 0) {
        console.log(`[${processed}/${towns.length}] ${t.name} → ${candidates.length} cands, ${(t.photos ?? []).length} photos${loc ? "" : " (no loc)"} [${mins}m]`);
  ...
  fs.writeFileSync(PROFILES_PATH, JSON.stringify(towns));
  const mins = Math.round((Date.now() - startTime) / 60000);
  console.log(`\nDone. Processed ${processed}, enriched ${enriched}, thin=${thin}, noLoc=${noLoc} in ${mins} min`);
}
